...
CDN으로 배포된다"는 한 줄로 요약됩니다. 동적 처리 단계가 전혀 없다는 점이 핵심입니다.
...
```text
 ...
 └──────┬─────────────────────────────────────────┘
        ...
        ▼
 ┌──────────────────────┐      ┌──────────────────┐
 │  Cloudflare Pages     │ ───▶ │  글로벌 CDN 엣지  │ ──▶ 방문자
 │  (정적 호스팅)         │      │  (캐시·HTTPS)     │
 └──────────────────────┘      └──────────────────┘
```
...
- GitHub 저장소를 연결하면 push/merge 시 자동으로 빌드·배포되는 Git 기반 워크플로를 제공합니다.
- 글로벌 CDN, 무료 HTTPS, 미리보기 배포(Preview Deployment), `public/_headers`를 통한 보안 헤더
  설정을 정적 사이트만으로 모두 지원합니다.
- 무료 등급으로 팬 프로젝트 규모를 충분히 감당하며, Workers·D1 같은 동적 기능 없이도 완결됩니다.
...
사이트맵은 Starlight가 `astro.config.mjs`의 최상위 `site` 키(`https://r-punghoe.pages.dev`)를 보고
**자동 생성**합니다. 빌드 시 `dist/`에 `sitemap-index.xml`과 `sitemap-0.xml`이 만들어지고, 모든
페이지 `<head>`에 사이트맵 링크가 추가됩니다. 별도 라이브러리를 추가할 필요가 없으며, `site` 키만
올바르게 유지하면 됩니다.

### 보안 헤더 (Security Headers)

보안 헤더는 `public/_headers` 파일에서 옵니다. `public/`의 파일은 빌드 시 `dist/`로 그대로 복사되고,
Cloudflare Pages가 이 `_headers` 파일을 읽어 응답 헤더로 적용합니다. 여기에는 CSP
